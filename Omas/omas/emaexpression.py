...
class EmaExpression(object):
    ...
    def __init__(self, docInfo, measures, staves, beats, completeness=None):
        ...
        mm_ranges = self.requested_measures.split(",")
        ...
        if len(staves_by_m) != len(mm_ranges) or \
           len(beats_by_m) != len(mm_ranges):
            e = "Requested staff/beat ranges do not match measure ranges."
            raise BadApiRequest(e)

        selections = []
        for i, m_range in enumerate(mm_ranges):
    ...
    def get(self):
        ...
        for sel in self.selections:
            ...
            for m_i, m in enumerate(sel.measureRange):
                ...
                ema_staves = []
                for s_i, s in enumerate(sel.staffRange):
                    ema_s = EmaStaff(number=int(s), measure=ema_m)
                    ema_beat_ranges = []
                    for br in sel.beatRange[m_i][s_i]:
...
class EmaSingleRangeExpression(object):
    ...
    def measureRange(self):
        ...
        return self._parseNumericRanges(request)
    ...
    def beatRange(self):
        ...
        for m in self.measureRange:
            ...
            if len(beats_by_staff) != len(self.staffRange):
